...
export default {
  name: 'ChordDiagram',
  mounted() {
    d3.json('111student_knowledge_analysis.json').then(data => {
      const leftColumn = d3.select("#left-column");
      const rightColumn = d3.select("#right-column");
      const tooltip = d3.select("#tooltip");

      const knowledgeSequence = ["r8S3g", "t5V9e", "m3D1v", "k4W1c", "y9W5d", "s8Y2f", "g7R2j", "b3C9s"];

      Object.keys(data).forEach((studentId, index) => {
        const studentData = data[studentId];
        const sequence = Array.from(new Set(studentData.map(d => d.knowledge)));

        const column = index % 2 === 0 ? leftColumn : rightColumn;

        const svgContainer = column.append("div").attr("class", "svg-container");
        ...
        const links = studentData.map(d => {
          const sourceNode = nodes.find(n => n.id === d.knowledge);
          const nextIndex = studentData.findIndex(s => s.knowledge_start_time === d.knowledge_end_time);
          const targetNode = nextIndex !== -1 ? nodes.find(n => n.id === studentData[nextIndex].knowledge) : null;
        ...
        const colorScale = d3.scaleSequential(d3.interpolateViridis)
          .domain([d3.min(links, d => d.start_time), d3.max(links, d => d.start_time)])
          .range(["#B1C9F4", "#355eaa"]);
        ...
        g.selectAll(".link")
          ...
          .attr("class", "link")
          .attr("d", d => {
            if ((d.source.id === "y9W5d" && d.target.id === "k4W1c") || (d.source.id === "y9W5d" && d.target.id === "s8Y2f") || (d.source.id === "s8Y2f" && d.target.id === "y9W5d") || (d.source.id === "k4W1c" && d.target.id === "y9W5d")) {
              return `M${d.source.x},${d.source.y} L${d.target.x},${d.target.y}`;
            }
            const dx = d.target.x - d.source.x;
            const isForward = knowledgeSequence.indexOf(d.source.id) < knowledgeSequence.indexOf(d.target.id);
            const dy = isForward ? -150 : 150;
            return `M${d.source.x},${d.source.y} Q${d.source.x + dx / 2},${d.source.y + dy} ${d.target.x},${d.target.y}`;
          })
...
.container {
  ...
  gap: 3px;
  width: 1450px; /* Set the desired container width */
  height: 400px; /* Set the desired container height */
