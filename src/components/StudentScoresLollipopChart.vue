...
export default {
  name: 'StudentScoresLollipopChart',
  mounted() {
    d3.csv('student_average_scores_updated.csv').then(data => {
      data.forEach(d => {
        ...
        d.question_3MwAFlmNO8EKrpY5zjUd_count = +d.question_3MwAFlmNO8EKrpY5zjUd_count;
      });
      ...
      svg.append("g")
         ...
         .on("mouseover", function(event, d) {
            tooltip.transition()
                   .duration(200)
                   .style("opacity", .9);
            tooltip.html(`ID: ${d.student_ID}<br>Knowledge Avg Score: ${d.knowledge_avg_score}<br>Question Avg Score: ${d.question_avg_score}<br>Knowledge Count: ${d.knowledge_t5v9e_count}<br>Question Count: ${d.question_3MwAFlmNO8EKrpY5zjUd_count}`)
