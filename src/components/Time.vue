  ...
  mounted() {
    ...
    d3.json('time.json').then(data => {
      ...
      data.forEach(student => {
        ...
        const studentContainer = container.append('div')
        ...
        const svg = studentContainer.append('svg')
        ...
        const hoursData = Array.from({ length: 24 }, (_, hour) => {
          ...
          const count = hourData ? d3.sum(hourData.questionCounts, d => d.count) : 0;
          return { hour, count };
        });
        ...
        const heatMap = svg.selectAll('.hour')
          ...
          .style('fill', d => d.count === 0 ? '#ffffff' : colorScale(d.count))
          .style('stroke', '#000')
...
#heatmap-container {
  width: 1000px; /* Set the desired container width */
