  ...
  createChart() {
    ...
    Promise.all([
      ...
      Object.keys(studentDataMap).forEach(studentID => {
        ...
        learnerData.forEach(d => {
          ...
          if (title) {
            ...
            if (!knowledgeMap[knowledge]) {
              knowledgeMap[knowledge] = { totalScore: 0, correctScore: 0, count: 0, sub_knowledge: {} };
            }
            if (!knowledgeMap[knowledge].sub_knowledge[sub_knowledge]) {
              knowledgeMap[knowledge].sub_knowledge[sub_knowledge] = { totalScore: 0, correctScore: 0, count: 0, titles: {} };
            }
            knowledgeMap[knowledge].totalScore += title.score;
            knowledgeMap[knowledge].correctScore += +d.score;
            knowledgeMap[knowledge].count += 1;
            knowledgeMap[knowledge].sub_knowledge[sub_knowledge].totalScore += title.score;
            knowledgeMap[knowledge].sub_knowledge[sub_knowledge].correctScore += +d.score;
            knowledgeMap[knowledge].sub_knowledge[sub_knowledge].count += 1;

            if (!knowledgeMap[knowledge].sub_knowledge[sub_knowledge].titles[d.title_ID]) {
              knowledgeMap[knowledge].sub_knowledge[sub_knowledge].titles[d.title_ID] = { count: 0, totalScore: 0, correctScore: 0, accuracySum: 0 };
            }
            knowledgeMap[knowledge].sub_knowledge[sub_knowledge].titles[d.title_ID].count += 1;
            knowledgeMap[knowledge].sub_knowledge[sub_knowledge].titles[d.title_ID].totalScore = title.score;
            knowledgeMap[knowledge].sub_knowledge[sub_knowledge].titles[d.title_ID].correctScore += +d.score;
            knowledgeMap[knowledge].sub_knowledge[sub_knowledge].titles[d.title_ID].accuracySum += +d.score / title.score;
          }
        });
        ...
        let subRadiusScale = d3.scaleSqrt()
          .domain([0, d3.max(knowledgeData, d => d3.max(d.subKnowledgeData, sk => sk.count))])
          .range([60, 85]);

        let tinyRadiusScale = d3.scaleSqrt()
          .domain([0, d3.max(knowledgeData, d => d3.max(d.subKnowledgeData, sk => d3.max(sk.titles, t => t.count)))])
          .range([10, 40]);
        ...
        let g = svg.selectAll("g")
          .data(knowledgeData)
          .enter().append("g")
          .attr("transform", (d, i) => `translate(${i * 480 + 240}, ${(height + 100) / 2})`);
        ...
        g.each(function(d) {
          let subG = d3.select(this).selectAll(".sub-circle-g")
          ...
          subG.append("circle")
            ...
            .on("mouseover", function(event, sk) {
              ...
              tooltip.transition()
                ...
                .style("left", (event.pageX + 10) + "px")  // 向右偏移10像素
                .style("top", (event.pageY + 10) + "px"); // 向下偏移10像素
          ...
          let tinyG = subG.selectAll(".tiny-circle-g")
            .data(sk => sk.titles)
            .enter().append("g")
          ...
          tinyG.append("circle")
            ...
            .on("mouseover", function(event, t) {
              ...
              tooltip.transition()
                .duration(200)
                .style("opacity", .9);
              tooltip.html(`题目ID: ${t.title_ID}<br>准确率: ${(t.accuracy * 100).toFixed(1)}%`)
          ...
          tinyG.append("text")
            .attr("class", "tiny-accuracy-label")
            .attr("dy", t => -tinyRadiusScale(t.count) * 0.8)
            .text(t => (t.accuracy * 100).toFixed(1) + "%");

          // Force simulation for sub-circles
          // eslint-disable-next-line no-unused-vars
          let simulation = d3.forceSimulation(d.subKnowledgeData)
            .force("charge", d3.forceManyBody().strength(80))
            .force("center", d3.forceCenter(0, 0))
            .force("collision", d3.forceCollide().radius(sk => subRadiusScale(sk.count) ))
            .on("tick", () => {
          ...
          subG.each(function(sk) {
            // eslint-disable-next-line no-unused-vars
            let tinySimulation = d3.forceSimulation(sk.titles)
              .force("charge", d3.forceManyBody().strength(8))
              .force("center", d3.forceCenter(0, 0))
              .force("collision", d3.forceCollide().radius(t => tinyRadiusScale(t.count) + 2))
              .on("tick", () => {
                d3.select(this).selectAll(".tiny-circle-g")
