...
export default {
  ...
  methods: {
    processData(data, chartID, datasetName) {
      ...
      const groupedByMajor = Array.from(
        d3.group(nestedData, (d) => d.major),
        ([key, value]) => ({
          major: key,
          students: value,
        })
      ...
      const knowledgeScores = d3.rollups(
        data,
        (v) => d3.sum(v, (d) => d.score) / v.length,
        (d) => d.knowledge
      );

      const knowledgePie = d3
        .pie()
        .sort(null)
        .value(() => 1);
